...
description: Calculate relationship affect ratios and coaching insights for relationship coaching data. Employee-facing tool for HIPAA-certified team members.
allowed-tools:
  - Bash
  - Read
  - Write
...
Calculate accurate affect ratios and extract coaching insights from relationship coaching data using SQL queries.
...
- Measure relationship health using affect ratio metrics
...
This skill focuses on **affect ratio calculations**, not schema navigation.
...
affect_counts AS (
  ...
  GROUP BY tp.person_id, tp.conversation_id
)
SELECT
  person_id,
  conversation_id,
...
affect_counts AS (
  SELECT
    ...
    COUNT(*) AS total_classified,
    STRING_AGG(DISTINCT cp.person_id::text, ' & ' ORDER BY cp.person_id::text) AS couple_ids
...
per_person_affects AS (
  ...
  WHERE m.provider_timestamp >= CURRENT_DATE - INTERVAL '56 days'
    AND m.provider_timestamp < CURRENT_DATE
  GROUP BY cp.person_id, cp.conversation_id
)
SELECT
  person_id,
  positive_count,
...
GROUP BY ci.person1_id, ci.person2_id;
```
...
SELECT affect, person_1, person_2, partnership FROM (
  SELECT
    'person_id' AS affect,
    (SELECT person1_id::text FROM couple) AS person_1,
    (SELECT person2_id::text FROM couple) AS person_2,
    'Both' AS partnership,
    0 AS sort_order
  UNION ALL
  SELECT
...
ORDER BY
  sort_order,
  CASE
    WHEN affect IN ('person_id', 'Period: Last 56 days') THEN 0
...
**Our Affect Categories:**

Our system uses **Partner-prefixed affects** for AI classification:
...
4. **Interpret results**: Apply research-based thresholds and check sample size warnings
...
```bash
#!/bin/bash
# File: arsenal/dot-claude/skills/relationship-analyst/affect_ratio.sh
...
1. **Announce usage**: "I'm using the relationship-analyst skill to calculate affect ratios..."
...
Compare affect ratios (positive/negative) on days with interventions vs withheld days.
...
user_daily_affects AS (
    SELECT uc.person_id, DATE(m.provider_timestamp AT TIME ZONE 'America/Los_Angeles') as msg_date, uc.conversation_id,
...
user_totals AS (
    SELECT uda.person_id, dc.day_type, SUM(uda.pos) as pos, SUM(uda.neg) as neg
...
user_ratios AS (
    ...
    FROM user_totals GROUP BY person_id
)
SELECT person_id,
...
FROM user_ratios ORDER BY person_id;
```
