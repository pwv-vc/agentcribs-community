...
**Health Thresholds:** Use research-based thresholds from "The cost of destructive conflict patterns" section.
...
WITH affect_counts AS (
  SELECT
    ...
    COUNT(*) FILTER (WHERE me.affect IN (
      'Partner-Affection', 'Partner-Validation', 'Partner-Enthusiasm',
      'Humor', 'Partner-Interest'
    )) as positive_count,
    COUNT(*) FILTER (WHERE me.affect IN (
      'Partner-Criticism', 'Partner-Contempt', 'Partner-Defensiveness',
      'Partner-Belligerence', 'Partner-Domineering', 'Partner-Threats',
      'Partner-Anger', 'Partner-Sadness', 'Partner-Fear / Tension',
      'Stonewalling'
    )) as negative_count
  FROM message m
...
SELECT
  ...
  END as affect_ratio
FROM affect_counts
ORDER BY week_start DESC, person_id;
```
...
WHERE m.provider_timestamp >= :current_week_start
  ...
  AND pc.person_id IN (:person1_id, :person2_id)
  AND (
    me.affect IN ('Partner-Affection', 'Partner-Contempt', 'Partner-Criticism', 'Stonewalling', 'Partner-Anger')
    OR me.conflict_state IN ('New Conflict', 'Escalation', 'De-escalation', 'De-Escalation', 'Resolution')
  )
ORDER BY
  CASE
    WHEN me.affect IN ('Partner-Criticism', 'Partner-Contempt', 'Stonewalling', 'Partner-Anger') THEN 1
