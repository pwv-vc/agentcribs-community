...
PGHOST=your-prod-db.cluster-xxxx.us-east-2.rds.amazonaws.com
PGDATABASE=yourproddb
...
-- NOTE: Use p.name in WHERE clause for lookup, but return p.id (never display names)
SELECT
...
```sql
-- Get messages with sender IDs (never display names - use p.id)
SELECT
  m.id,
